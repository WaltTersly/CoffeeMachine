...
def sufficient_resource(order_made):
    """Returns True when order can be made,
    False if ingredients are insufficient .
    """
    for item in order_made:
        if order_made[item] >= resources[item]:
...
def is_transcations_successful(money_recieved, drink_cost):
    """Return True if the payment is accepted,
    False if the money recieved is insufficient

    Args:
        money_recieved (_type_): _description_
...
while is_on:
    ...
    if choice == "off":
        ...
        if sufficient_resource(drink["ingredients"]):
            ...
            if is_transcations_successful(payment, drink["cost"]):
